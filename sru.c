...
void print1(float *array, int time_step, int row, int col)
{
    int i, j, k;
...
void sru_create_instance(void* buf, sru_handle* handle, int hidden_dim, int batch_size, int time_step) {
    handle->A = (const float**)buf;
    handle->B = (const float**)(buf + 4 * time_step * sizeof(float*));
    ...
    handle->x_tmp_t = (float*)((void*)handle->r_t + time_step * batch_size * hidden_dim * sizeof(float));
    handle->c_t = (float*)((void*)handle->x_tmp_t + time_step * batch_size * hidden_dim * sizeof(float));
